...
var SalesQRScanner = /*#__PURE__*/function () {
  ...
  return _createClass(SalesQRScanner, [{
    ...
    value: function showSuccessState(message) {
      this.updateStatus("\n            <div class=\"text-center\">\n                <p class=\"text-lg font-semibold text-emerald-600\">".concat(message, "</p>\n                <p class=\"mt-1 text-sm text-slate-500\">Ready for the next fish box scan.</p>\n            </div>\n        "));
    }
  }, {
    key: "showErrorState",
    ...
    value: function () {
      var _handleScanResult = _asyncToGenerator(/*#__PURE__*/_regenerator().m(function _callee4(qrCode) {
        var fishBox, fishBoxData, message, _t2;
        return _regenerator().w(function (_context4) {
          while (1) switch (_context4.p = _context4.n) {
            ...
            case 4:
              fishBox = _context4.v;
              fishBoxData = fishBox.data || fishBox;
              if (!this.isFishBoxAlreadySelected(fishBoxData.id)) {
                _context4.n = 5;
                break;
              }
              message = 'This fish box is already in the current transaction.';
              if (window.toastr) {
    ...
    value: function restartScannerAfterSuccess(message) {
      ...
      this.restartTimeout = setTimeout(function () {
        ...
        if (!_this5.modal || _this5.modal.classList.contains('hidden')) {
          return;
        }
        _this5.startScanner()["catch"](function (error) {
          _this5.showErrorState(_this5.getCameraErrorMessage(error));
        });
      }, 700);
    }
  }, {
    ...
    value: function getActiveSalesFormRoot() {
      var fullPageRoot = document.querySelector('[data-sales-form-root]');
      if (fullPageRoot) {
        return fullPageRoot;
      }
    ...
    key: "isFishBoxAlreadySelected",
    value: function isFishBoxAlreadySelected(fishBoxId) {
      if (!fishBoxId) {
        return false;
      }
      var salesFormRoot = this.getActiveSalesFormRoot();
      return Array.from(salesFormRoot.querySelectorAll('.fish-box-hidden-input, input[type="hidden"][name*="[box_id]"]')).some(function (input) {
        return String(input.value) === String(fishBoxId);
      });
    }
  }, {
    key: "getNextRowIndex",
    value: function getNextRowIndex(container) {
      var indexes = Array.from(container.querySelectorAll('.sales-detail-row')).map(function (row) {
        return Number.parseInt(row.dataset.index, 10);
      }).filter(Number.isFinite);
      return indexes.length > 0 ? Math.max.apply(Math, _toConsumableArray(indexes)) + 1 : 0;
    }
  }, {
    key: "getScannedUnitPrice",
    value: function getScannedUnitPrice(fishBox) {
      var _ref, _ref2, _ref3, _fishBox$unit_price;
      var price = (_ref = (_ref2 = (_ref3 = (_fishBox$unit_price = fishBox.unit_price) !== null && _fishBox$unit_price !== void 0 ? _fishBox$unit_price : fishBox.price) !== null && _ref3 !== void 0 ? _ref3 : fishBox.suggested_price) !== null && _ref2 !== void 0 ? _ref2 : fishBox.latest_price) !== null && _ref !== void 0 ? _ref : null;
      var numericPrice = Number(price);
      return Number.isFinite(numericPrice) && numericPrice > 0 ? numericPrice : null;
    }
  }, {
    key: "ensureFishTypeOption",
    value: function ensureFishTypeOption(fishTypeSelect, fishTypeId, fishTypeName) {
      var suggestedPrice = arguments.length > 3 && arguments[3] !== undefined ? arguments[3] : null;
      if (!fishTypeSelect || !fishTypeId) {
        return null;
      }
      var normalizedFishTypeId = String(fishTypeId);
      var option = Array.from(fishTypeSelect.options).find(function (candidate) {
        return candidate.value === normalizedFishTypeId;
      });
      if (!option) {
        option = new Option(fishTypeName || "Fish #".concat(normalizedFishTypeId), normalizedFishTypeId);
    ...
    key: "applyScannedUnitPrice",
    value: function applyScannedUnitPrice(targetRow, suggestedPrice) {
      if (suggestedPrice === null) {
        return;
      }
      var unitPriceInput = targetRow.querySelector('.unit-price-input');
      if (!unitPriceInput) {
        return;
      }
      var currentValue = Number(String(unitPriceInput.value || '').replace(/[₱,\s]/g, ''));
    ...
    value: function addFishBoxToSalesDetails(fishBox) {
      ...
      var scannedUnitPrice = this.getScannedUnitPrice(fishBox);
